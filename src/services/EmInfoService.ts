...
export class EmService {
    ...
    private static readonly CODE_NAME_MAP: Record<string, string> = {
        ...
        'f116': '总市值', 'f117': '流通市值', 'f189': '上市时间', 'f127': '所属行业', 'f128': '地域板块',
    };
    ...
    static async getStockInfo(symbol: string): Promise<Record<string, any>> {
        ...
        if (!response.ok) throw new Error(`东方财富接口请求失败: ${response.status}`);

        const json: any = await response.json();
        const innerData = json.data;
        if (!innerData) throw new Error('东方财富接口返回数据格式异常');

        const result: Record<string, any> = { '市场代码': identity.market.toUpperCase() };
        for (const [key, name] of Object.entries(this.CODE_NAME_MAP)) {
            if (key in innerData) result[name] = innerData[key];
        }
        return result;
    }
